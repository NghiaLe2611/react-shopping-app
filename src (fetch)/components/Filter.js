...
const Filter = (props) => {
    ...
    const handleFilterBrand = (e, brandName) => {
        ...
        if (filter.brand.includes(brandName)) {
            setFilter(obj => obj = {...obj, brand: obj.brand.filter(item => item !== brandName)});
        } else {
            setFilter(obj => obj = {...obj, brand: [...obj.brand, brandName]});
        }
        setActiveFilter(null);
    };
    ...
    const filterClassCriteria = (filterName) => {
        switch (filterName) {
            case 'brand': 
                return {
                    borderColor: filter.brand.length || activeFilter === filterName ? '#ee4d2d' : ''
                }
            case 'os': 
                return {
            ...
            <ul className={`${classes['filter-list']} ${classes['list-img']}`}>
                {
                    brandSmartphone.map((item) => (
                        <li key={item.brand} className={filter.brand.includes(item.brand) ? classes.active : ''}>
        ...
        <div className={classes.filter} ref={componentRef}>
            ...
            <div className={classes['filter-item']}>
                <span className={classes.name} style={filterClassCriteria('brand')} onClick={() => showFilter('brand')}>
                    { !filter.brand.length && 'Hãng' }
                    { filter.brand.length === 1 && filter.brand[0]}
                    { filter.brand.length > 1 && `${filter.brand[0]},...`}
                </span>
