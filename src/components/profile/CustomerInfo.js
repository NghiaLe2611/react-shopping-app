...
const yearList = Array.from(Array(new Date().getFullYear() - 1899), (_, i) =>
	(i + 1900).toString()
).reverse();
...
const CustomerInfo = (props) => {
	...
	const updateUserData = (e) => {
		...
		if (
			...
			selectedMonth &&
			selectedYear &&
			oldBirthday.getTime() !== newBirthday.getTime()
		) {
				...
				<div className={classes.right}>
					...
					<div className={classes.group}>
						...
						<div className={classes.item}>
							...
							{emailVerified ? (
								<button
									className={`${classes.update} ${classes.active}`}>
									Đã liên kết
								</button>
							) : (
