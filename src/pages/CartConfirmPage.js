...
import {formatCurrency, convertProductLink, convertCardNumber, convertCardExpiry, shippingFee, fastShippingFee, getDayName} from '../helpers/helpers';
								...
								<div className={classes['wrap-shipping']}>
									<div className={classes['shipping-method']}>
										<label className={classes.checkbox}>
											<input type='radio' name='shipping_method' value='1' defaultChecked={shippingMethod} onChange={onChangeShippingMethod} />
											...
											cart.finalItems.map((item, index) => (
												<div key={item._id} className={classes.item}>
													...
													<div className={classes['shipping-info']}>
														<p className={classes.time}>{shippingMethod === 1 ? 'Giao trước 11:59 sáng mai' : `Giao vào ${getDayName()}`}</p>
														<p>
															{shippingMethod === 1 ? (
																<img src={iconFastShipping} className={classes.fast} alt='fast-shipping' />
															) : (
													...
													{index === 0 && (
														<div className={`${classes['shipping-fee']} ${classes.all}`}>
															{shippingMethod === 1 ? (
																`${formatCurrency(fastShippingFee)}đ`
															) : (
																<Fragment>
																	<var>{formatCurrency(fastShippingFee)}đ</var>
																	<br />
							...
							<div className={classes.box}>
								<ul className={classes['list-payment']}>
									{listPayment.map((item) => (
										<li key={item.id}>
											<label className={classes.checkbox}>
												<input type='radio' name='payment_method' value={item.id} defaultChecked={item.id === paymentMethod ? true : false} onChange={onChangePaymentMethod} />
												...
												<div className={classes['form-left']}>
													<div className={classes['input-group']}>
														...
														<input
															...
															className={cardError['card_number'] ? classes.invalid : null}
														/>
													...
													<div className={classes['input-group']}>
														...
														<input
															...
															maxLength='50'
															className={cardError['card_name'] ? classes.invalid : null}
															onFocus={focusCardInput}
															onChange={onChangeCardInput}
															onBlur={onBlurInput}
															value={cardInfo['card_name']?.toUpperCase() || ''}
														/>
													...
													<div className={classes['input-group']}>
														...
														<input
															type='text'
															name='card_expiry'
															className={`${classes.short} ${cardError['card_expiry'] ? classes.invalid : null}`}
															placeholder='MM/YY'
															onFocus={focusCardInput}
															onChange={onChangeCardInput}
													...
													<div className={classes['input-group']}>
														...
														<input
															type='text'
															name='card_cvv'
															className={`${classes.short} ${cardError['card_cvv'] ? classes.invalid : null}`}
															placeholder='VD: 123'
															onFocus={focusCardInput}
															onChange={onChangeCardInput}
															onBlur={onBlurInput}
							...
							<div className={classes.block}>
								<div className={classes['wrap-ctm-info']}>
									...
									{customerInfo ? (
										...
										<span className={classes['add-address']} onClick={showAddressModalHandler}>
								...
								<div className={classes['wrap-summary']}>
									<div className={classes.head}>
										<div className={classes['head-title']}>
											<b>Đơn hàng</b>
											<p>
												{cart.finalItems.length} sản phẩm.&nbsp;
									...
									<div className={classes['price-summary']}>
										...
										<p>
											...
											<span>{shippingMethod === 1 ? formatCurrency(fastShippingFee) : formatCurrency(shippingFee)}đ</span>
										</p>
										{cart.appliedCoupons
										...
										<p className={classes['total-price']}>
											...
											<span className={classes.total}>{cart.finalPrice > 0 ? formatCurrency(cart.finalPrice) : formatCurrency(cart.totalPrice)} ₫</span>
										</p>
									</div>
								</div>
							</div>
